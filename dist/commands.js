...
async function handleSlashCommand(ctx, data, member, cmdCtx, interactionChannelId) {
    ...
    switch (subName) {
        ...
        case "approve":
            return handleApprove(ctx, getOption(subcommand.options ?? [], "id"), member?.user.username, baseUrl);
        case "budget":
...
async function handleApprove(ctx, approvalId, username, baseUrl) {
    if (!approvalId) {
        return respondToInteraction({
            type: 4,
...
async function handleEscalationButton(ctx, customId, actor, _baseUrl) {
    ...
    const record = await getEscalation(ctx, escalationId, resolvedCompanyId);
    if (!record) {
...
async function handleCommands(ctx, subcommandGroup, cmdCtx) {
    ...
    switch (sub.name) {
        ...
        case "run":
            return handleCommandsRun(ctx, companyId, baseUrl, token, channelId, getOption(sub.options ?? [], "name") ?? "", getOption(sub.options ?? [], "args") ?? "");
        case "delete":
...
async function handleCommandsRun(ctx, companyId, baseUrl, token, channelId, name, args) {
    if (!name.trim()) {
        return respondToInteraction({
            type: 4,
...
async function handleWorkflowApprovalButton(ctx, customId, actor, cmdCtx) {
    ...
    const result = await resumeWorkflowAfterApproval(ctx, token, channelId, companyId, baseUrl, approvalId, approved);
